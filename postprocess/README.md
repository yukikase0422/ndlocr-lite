...
生成ファイルは `.ndlocr-body.txt` / `.ndlocr-index.json` / `.ndlocr-indexed.txt` のサフィックスで識別できる。このサフィックスは Claude Code 等の自動化ツールが当該ファイルを判別するための一意な識別子として機能する。

設計の経緯と仕様は [Issue #1](https://github.com/yukikase0422/ndlocr-lite/issues/1)（後処理ツール本体）および [Issue #2](https://github.com/yukikase0422/ndlocr-lite/issues/2)（命名規則・Claude Code連携）を参照。
...
`--prefix` は必須。出力ファイル名の頭に付く書籍識別子（例: `kokusaiho_2nd`）。

生成物：

| ファイル | 用途 |
|---|---|
| `<prefix>.ndlocr-body.txt` | **検索専用** 本文のみを全ページ通して連結、ページマーカーなし |
| `<prefix>.ndlocr-index.json` | バイトオフセット→ページ番号の対応表 |
| `<prefix>.ndlocr-indexed.txt` | **人間閲覧用** ページマーカー `<<<PAGE N>>>` と章見出しマーカー入り |

### 3. 検索

```bash
# ディレクトリ指定（中の *.ndlocr-body.txt を自動検出）
python postprocess/search_ocr.py <merged_output_dir> "検索語"

# ファイル直接指定
python postprocess/search_ocr.py <merged_output_dir>/<prefix>.ndlocr-body.txt "検索語"

# 同一ディレクトリに複数の .ndlocr-body.txt がある場合
python postprocess/search_ocr.py <merged_output_dir> "検索語" --prefix <prefix>
```
