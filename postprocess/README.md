...
| オプション | 説明 |
|---|---|
| `--skip-ocr` | NDLOCR実行をスキップし、後処理のみ行う（デバッグ・集約のみ再生成用） |
| `--dpi N` | PDF画像化時のDPI（既定: 300） |
| `--force-rerender` | PDF画像化を強制再実行（既存 `page_N.png` を無視） |
| `--force-reocr` | NDLOCR実行を強制再実行（既存 `page_N.xml/txt/json` を無視） |

---

## 中断再開（resume）動作

`build_merged_text.py` は、長時間のOCR処理が途中で中断（セッションクラッシュ等）した場合の**再開に対応**している。同じ引数で再実行すると、既存の中間物を可能な限り再利用する。Issue #4 で実装。

### 各フェーズの挙動

| フェーズ | 既存ファイルの扱い | 強制再実行フラグ |
|---|---|---|
| PDF画像化 | `page_N.png` が存在しサイズ > 0 のページはスキップ | `--force-rerender` |
| NDLOCR実行 | **全ページの `.xml`/`.txt`/`.json` が揃っていれば完全スキップ**。一部でも欠けていれば全量再実行（NDLOCR本家が既存出力を上書きする挙動に依存。Issue #4「案1」採用） | `--force-reocr` |
| 集約（3種テキスト生成） | **常に中間物の全量から再生成（上書き）**。部分的な中間物で集約ファイルが中途半端な状態で残るのを防ぐ | なし（常時実行） |

### 再開時のユースケース

- **ユースケース1（画像化完了後に中断）**: 再実行すると画像化はスキップし、OCRから再開
- **ユースケース2（OCR途中で中断）**: 再実行すると全量OCR再実行（本家が既存出力を上書き）
- **ユースケース3（OCR完了、集約ファイル破損）**: 再実行すると OCR もスキップ、集約のみ再生成
- **ユースケース4（集約のみ再生成したい）**: `--skip-ocr` で中間物から集約のみ生成

### OCR部分スキップを採用しなかった理由

「未処理ページのみ一時ディレクトリに退避→NDLOCR実行→結果をマージ」方式（Issue #4「案2」）は実装が複雑になるため、**案1（全量実行、本家が既存を上書き）を採用**。NDLOCR処理時間が長い場合は全量再実行のコストが重いが、案2の実装・保守コストとのトレードオフで案1を選択した。将来的に再検討する余地あり。
